               ...
               const char *buf, int bufLen, int bufPaddingSize, int hdrLen)
{
    ...
    // Guard against UB: memcpy(dst, NULL, 0) is undefined even when
    // count==0 and crashes under Zig's safety-checked C compilation.
    // _createControlMessage() passes NULL,0 for control messages.
    if (subject != NULL)
